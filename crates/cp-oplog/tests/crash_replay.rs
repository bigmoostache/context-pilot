...
mod tests {
    ...
    fn child_command(dir: &Path) -> ! {
        let mut writer = OplogWriter::open(dir).unwrap_or_else(|e| panic!("child open: {e}"));
        let kind =
            OpEntryKind::CommandEffect { cmd_id: "deadman-cmd".to_owned(), dedup_token: COMMAND_TOKEN.to_owned() };
    ...
    /// `dir` in `mode`. The test is now nested in `mod tests`, so the `--exact`
    /// filter carries the module path.
    fn spawn_child(dir: &Path, mode: &str) -> std::process::Child {
        let exe = env::current_exe().unwrap_or_else(|e| panic!("current_exe: {e}"));
        Command::new(exe)
            .args(["--exact", "tests::crash_child_entrypoint", "--nocapture"])
    ...
    fn wait_for_marker_lines(dir: &Path, n: usize) -> Vec<String> {
        let deadline = Instant::now().checked_add(Duration::from_secs(10)).expect("deadline in range");
        loop {
