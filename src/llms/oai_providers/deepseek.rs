...
impl LlmClient for DeepSeekClient {
    fn stream(&self, request: LlmRequest, tx: Sender<StreamEvent>) -> Result<(), LlmError> {
        // Gateway when configured, DeepSeek directly otherwise; resolved before
        // the key check because a gateway deployment keeps its provider keys
        // in the proxy, not in this process's vault.
        let target = crate::llms::gateway::oai_target(DEEPSEEK_API_ENDPOINT, self.api_key.as_ref())
    ...
    fn check_api(&self, model: &str) -> super::super::ApiCheckResult {
        ...
        // client's `check_api`.
        let Some(target) = crate::llms::gateway::oai_target(DEEPSEEK_API_ENDPOINT, self.api_key.as_ref()) else {
