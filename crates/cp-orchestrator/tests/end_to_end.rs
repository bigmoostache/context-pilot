...
mod tests {
    ...
    fn a_command_journals_on_the_agent_and_re_emerges_as_an_sse_delta() {
        ...
        let agent = spawn_agent_serving_one_command(&sock, &oplog_dir, cap_token);
        ...
        assert_command_receipt(&receipt);
        ...
        assert_delta_carries_command(&events);
    }
    ...
    fn the_stream_survives_a_soak_of_connect_disconnect_cycles() {
        ...
        for _cycle in 0..25u32 {
            ...
            let (status, events) = common::sse_collect(
                ...
                &path,
                &[("Last-Event-ID", "0")],
                common::SseWait { want: 1, deadline: Duration::from_secs(2) },
            );
    ...
    /// Assert the `POST /command` receipt echoes the command id, dedup token,
    /// accepted status, and the durable `rev` the effect landed at. Extracted
    /// from the capstone test to keep its body under the cognitive-complexity cap.
    fn assert_command_receipt(receipt: &common::HttpResponse) {
    ...
    /// Assert the streamed events carry the journalled command back as a `delta`
    /// tagged with its rev and bearing the command's dedup token — the full loop
    /// closed. Extracted from the capstone test for the cognitive-complexity cap.
    fn assert_delta_carries_command(events: &[common::SseEvent]) {
    ...
    /// Spawn the agent side of the capstone loop: bind the UDS, then serve
    /// exactly one command connection on a background thread (real oplog +
    /// intake), and return its join handle. Extracted from the capstone test to
    /// keep that test's body under the cognitive-complexity cap.
    fn spawn_agent_serving_one_command(sock: &Path, oplog_dir: &Path, cap_token: &str) -> thread::JoinHandle<()> {
        let listener = UnixListener::bind(sock).expect("bind socket");
        let oplog_dir_owned = oplog_dir.to_path_buf();
        let cap_token_owned = cap_token.to_owned();
        thread::spawn(move || {
            let oplog = OplogService::spawn(&oplog_dir_owned).expect("spawn oplog");
            let mut intake = Intake::new(&oplog_dir_owned, cap_token_owned).expect("intake");
