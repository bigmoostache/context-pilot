...
fn handle(mut request: Request, state: &Arc<Mutex<Backend>>) {
    ...
    if method == Method::Get && segments.first() != Some(&"api") && web_root().is_some() {
...
fn web_root() -> Option<&'static PathBuf> {
    ...
    WEB_ROOT
        .get_or_init(|| std::env::var_os("CP_WEB_ROOT").map(PathBuf::from).filter(|p| p.is_dir()))
        .as_ref()
}
...
fn serve_static(request: Request, path: &str) {
    ...
    let (file, is_asset) = if candidate.is_file() {
        (candidate, rel.starts_with("assets/"))
    } else {
