...
mod tests {
    ...
    /// Assert a kind classifies as best-effort. A plain call, so folding the
    /// classification asserts through it keeps the test under the cognitive cap.
    fn assert_best_effort(kind: &OpEntryKind) {
        assert_eq!(Durability::of(kind), Durability::BestEffort);
    }

    /// Assert a kind classifies as durable (plain call — see `assert_best_effort`).
    fn assert_durable(kind: &OpEntryKind) {
        assert_eq!(Durability::of(kind), Durability::Durable);
    }

    /// Assert the collected revs are unique and exactly `0..total`, contiguous.
    /// Extracted from the concurrency test to keep it under the cognitive cap.
    fn assert_unique_contiguous(all_revs: &[u64], total: usize) {
        assert_eq!(all_revs.len(), total, "every durable append returned a rev");
        let mut sorted = all_revs.to_vec();
    ...
    /// Assert replay recovers exactly `total` committed effects with the right head.
    /// Extracted from the concurrency test to keep it under the cognitive cap.
    fn assert_replay_all_committed(dir: &Path, total: usize) {
        let recovered = replay(dir).expect("replay");
    ...
    /// Drive a best-effort flood, returning `(submitted, dropped)`. Extracted from
    /// the flood test so its loop + branch don't push it over the cognitive cap.
    fn run_best_effort_flood(service: &Service, flood: u64) -> (u64, u64) {
    ...
    /// Assert the service still commits + replays a durable record after a flood.
    /// Extracted from the flood test to keep it under the cognitive cap.
    fn assert_service_healthy_after_flood(dir: &Path, rev: u64) {
        let recovered = replay(dir).expect("replay");
    ...
    fn durability_of_is_exhaustive_over_every_kind() {
        ...
        assert_best_effort(&phase());
        assert_best_effort(&OpEntryKind::CostAggregate { input_tokens: 1, output_tokens: 2, cost_usd: 0.5f64 });
        ...
        assert_durable(&effect("c"));
        assert_durable(&OpEntryKind::SeenMark { dedup_token: "d".to_owned() });
        assert_durable(&msg("T1", 1));
        assert_durable(&OpEntryKind::Lifecycle { state: LifecycleState::Running });
        assert_durable(&OpEntryKind::Checkpoint { snapshot: Snapshot::default() });
        assert_durable(&OpEntryKind::Unknown);
    }
    ...
    fn concurrent_durable_submitters_get_unique_contiguous_revs() {
        ...
        assert_unique_contiguous(&all_revs, total);

        // Replay confirms the durable truth: every token landed exactly once.
        assert_replay_all_committed(dir.path(), total);
    }
    ...
    fn best_effort_floods_drop_without_ever_blocking_the_submitter() {
        ...
        let (submitted, dropped) = run_best_effort_flood(&service, flood);
        ...
        assert_service_healthy_after_flood(dir.path(), rev);
    }
