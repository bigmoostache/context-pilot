...
|---|---------|-------|------------------|---------------|
| 1 | **Flashing the wrong disk.** Kernel probe order is not stable, so a hard-coded `/dev/mmcblk0` can point at the SD or a reader, nuking it. | `init-sd-install.sh` | eMMC is detected by `removable==0` **and** must differ from the booted device; refuses on ambiguity or if it equals the boot disk. | If a board exposes the eMMC as `removable=1` (unusual), detection fails safe (aborts) rather than guessing — needs a real-hardware check of `/sys/block/mmcblk*/removable`. |
...
| 3 | **LCD painter crashes on the real OS.** Debian 13 ships **libgpiod 2.x**; v1 used the v1 API and a flat gpiochip0 offset → immediate crash, no progress screen. | `pcat_lcd.py` | v2/v1 adapter; line resolved by **name** across all gpiochips with the flat offset only as fallback; every path is exception-wrapped and returns 0 so a dead LCD never aborts the flash. | The RK line **names** (`GPIO3_C2` etc.) are best-guess; verify against `gpioinfo` on the board. Power-off remains the authoritative done-signal regardless. |
