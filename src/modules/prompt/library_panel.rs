...
impl Panel for LibraryPanel {
    ...
    fn content(&self, state: &State, base_style: Style) -> Vec<Line<'static>> {
        ...
        let agent_rows: Vec<Vec<Cell>> = state.agents.iter().map(|agent| {
            ...
            let (type_str, type_color) = if agent.is_builtin {
        ...
        if !state.skills.is_empty() {
            ...
            let skill_rows: Vec<Vec<Cell>> = state.skills.iter().map(|skill| {
                ...
                let (type_str, type_color) = if skill.is_builtin {
                    ("built-in", theme::accent_dim())
                } else {
                    ("custom", theme::success())
                };
        ...
        if !state.commands.is_empty() {
            ...
            let cmd_rows: Vec<Vec<Cell>> = state.commands.iter().map(|cmd| {
                let (type_str, type_color) = if cmd.is_builtin {
                    ("built-in", theme::accent_dim())
                } else {
                    ("custom", theme::success())
                };
