...
/**
 * Create Command dialog — a focused shadcn sheet for authoring a new `/command`
 * in the active agent's prompt library (T350). Opened from the "create command"
 * pill beside the composer's suggestion bubbles.
 *
 * Three fields: a **name** (whose slug becomes the `/invocation`, previewed
 * live as you type), an optional one-line **description** (the bubble label),
 * and the **prompt body** the command expands to. A disabled replica of the
 * suggestion bubble previews exactly what will appear in the composer. On submit
 * the command is written via {@link useCreateCommand}; the agent picks the file
 * up on its filesystem watch and the new bubble surfaces on the next library
 * refetch. A duplicate name surfaces the backend's 409 inline.
 *
 * Built on the portaled Base UI Dialog primitive (focus-trap, scroll-lock,
 * Esc-to-close, backdrop fade + spring-in) — no hand-rolled overlay.
 */
