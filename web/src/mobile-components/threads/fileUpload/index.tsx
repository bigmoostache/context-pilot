...
/**
 * Existence-checking wrapper around {@link FileUploadChip} — the mobile twin of
 * `components/threads/fileUpload/index`.
 *
 ...
 *
 * Divergence from desktop is touch-only: the chip hover states become `active`
 * (touch has no hover) and the mac-icon helpers resolve through the mobile tree.
 */
 ...
 * and its on-disk size; tapping opens the shared Finder Quick Look drawer
 * (`QuickLookSheet`) — the whole card is the tap target.
 *
 * Three presentations:
...
export function FileUploadChip({
  ...
  const base =
    "group/chip inline-flex max-w-full items-center gap-2.5 rounded-xl border px-2.5 py-2 text-left align-middle transition-[border-color,background-color,transform,box-shadow]"
  const skin = onAccent
    ? "border-white/25 bg-white/12 active:bg-white/20"
    : "border-border/70 bg-card card-shadow active:border-(--signal)/55 active:bg-muted/30"
  ...
  // Both carry no glyph; the whole card is the tap target when interactive.
...
function MissingChip({ file, onAccent }: { file: UploadedFile; onAccent: boolean }) {
  return (
    <span
      className={
        onAccent
          ? "inline-flex max-w-full items-center gap-2.5 rounded-xl border border-dashed border-white/25 bg-white/8 px-2.5 py-2 text-left align-middle opacity-80"
          : "inline-flex max-w-full items-center gap-2.5 rounded-xl border border-dashed border-border bg-muted/30 px-2.5 py-2 text-left align-middle opacity-80"
      }
 ...
 *  branches toward the P8 complexity budget). Sized to a 32px touch target. */
function ShowInFinderButton({
  ...
  return (
    <button
      ...
      className={
        onAccent
          ? "flex size-8 shrink-0 items-center justify-center rounded-lg opacity-55 transition-opacity active:opacity-100"
          : "flex size-8 shrink-0 items-center justify-center rounded-lg text-muted-foreground/45 transition-colors active:bg-muted/60 active:text-foreground/80"
      }
    >
      ...
      {files.map((f, i) => (
        <span key={`${f.path}-${i}`} className="inline-flex items-center gap-1">
          ...
          <button
            ...
            className="flex size-5 items-center justify-center rounded-full bg-muted text-muted-foreground/70 transition-colors active:bg-destructive/20 active:text-destructive"
