...
pub(crate) fn render_uplink_env(config: &NetworkConfig) -> String {
    ...
    // `writeln!` to a String is infallible; the #[must_use] Result is bound
    // (workspace forbids both unused_must_use and let_underscore_must_use).
    let _w1 = writeln!(out, "CP_UPLINK_MODE={}", config.mode.as_str());
    let _w2 = writeln!(out, "CP_UPLINK_WAN_IF={}", wan_iface());
    let _w3 = writeln!(out, "CP_UPLINK_WWAN_PROFILE={WWAN_PROFILE}");
    let _w4 = writeln!(out, "CP_UPLINK_WWAN_DEV={}", wwan_device());
    let _w5 = writeln!(out, "CP_UPLINK_STANDBY={}", config.wwan.standby.as_str());
    let _w6 = writeln!(out, "CP_UPLINK_TARGETS=\"{}\"", config.probe.targets.join(" "));
    let _w7 = writeln!(out, "CP_UPLINK_FAIL_THRESHOLD={}", config.probe.fail_threshold);
    let _w8 = writeln!(out, "CP_UPLINK_OK_THRESHOLD={}", config.probe.ok_threshold);
    let _w9 = writeln!(out, "CP_UPLINK_INTERVAL_S={}", config.probe.interval_s);
    let _w10 = writeln!(out, "CP_UPLINK_PROBE_TIMEOUT_S={}", config.probe.probe_timeout_s);
    let _w11 = writeln!(out, "CP_UPLINK_COOLDOWN_S={}", config.probe.cooldown_s);
    let _w12 = writeln!(out, "CP_UPLINK_NM_WAIT_S={}", config.probe.nm_wait_s);
    let _w13 = writeln!(out, "CP_UPLINK_METRIC_PREFERRED={}", super::apply::METRIC_PREFERRED);
    let _w14 = writeln!(out, "CP_UPLINK_METRIC_STANDBY={}", super::apply::METRIC_STANDBY);
    out
}
