...
/**
 * SMS on the box's own SIM — mobile twin of
 * `components/shell/config/it/ItSmsPane`.
 *
 * Divergence from desktop is touch-only: the "Load older", "Remove" and "Send"
 * buttons carry a taller hit area, `hover:` becomes `active:`, and an outbound
 * card is indented less because the viewport is narrower. Everything else — the
 * two queries, the gate on `status.sms`, the mutations, the compose form — is
 * byte-identical to the desktop twin, because every rule behind it lives in the
 * shared `@/lib/api/it/sms` module rather than being forked (C8).
 *
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {list.hasNextPage && (
        <button
          ...
          className="self-start rounded-md px-3 py-1.5 text-[11px] text-muted-foreground transition-colors active:bg-muted/60 active:text-foreground disabled:opacity-50"
        >
  ...
  return (
    <div
      className={cn(
        ...
        view.inbound ? "border-border bg-muted/40" : "ml-4 border-(--interactive)/30 bg-card",
      )}
    >
      ...
      <div className="flex items-center gap-2">
        ...
        {open && (
          <button
            ...
            className="ml-auto flex items-center gap-1 rounded-md px-2.5 py-1.5 text-[11px] text-muted-foreground transition-colors active:bg-(--danger)/10 active:text-(--danger) disabled:opacity-50"
          >
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex items-center gap-1.5 rounded-md bg-(--interactive) px-3.5 py-2 text-[12px] font-medium text-(--primary-foreground) transition-all active:brightness-105 disabled:opacity-50"
        >
