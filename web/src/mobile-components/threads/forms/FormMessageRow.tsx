...
//
// The mobile twin of `components/threads/forms/FormMessageRow`. A message
// carrying a ` ```form ` or ` ```form-answer ` block renders through here rather
// than the memoized plain MessageRow, so it re-renders when a matching
// `form-answer` lands and flips the widget to its locked state (docs/forms.md
// §4/§5).
//
// Identical structure to the desktop twin; the only fork is that `Message` and
// `FormWidget` resolve through the `@/mobile-components` tree (the mobile message
// bubble + the mobile field renderers). The form parse/strip helpers are shared
// logic (`./helpers`, a keep-stub re-export of the desktop parsers).
