...
/**
 * Access-point form — mobile twin of `components/shell/config/it/ItApForm`.
 *
 * Divergence from desktop is touch-only: the band/channel/country row stacks
 * instead of sitting in three columns (it does not fit a 390px viewport), the
 * `<select>` carries a **16px font** (iOS Safari auto-zooms the viewport on
 * focus below 16px), and the submit button grows and swaps `hover:` for
 * `active:`. The draft handling, the validation and the mutation are
 * byte-identical; they live in `@/lib/api/it/network`.
 *
 * The passphrase is write-only — the server never sends it
