 ...
 * (no dialog chrome of its own). Mobile twin of `components/shell/SessionVitals`.
 *
 * Rendered inside the Agent Configuration surface (manage mode). The backend
 ...
 *
 * Divergence from desktop is touch-only: the probe grid is forced to a **single
 * column** (the desktop `sm:grid-cols-2` two-up wastes width and cramps rows on
 * a phone), and the Check button + hover states are sized/retuned for touch
 * (`hover:` becomes `active:`, larger padding). All probe logic — the frontend
 * round-trip prepend, category grouping, status colouring — is identical.
 */
      ...
      <div className="flex flex-col gap-3.5">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              disabled={checking}
              className="flex shrink-0 items-center gap-1.5 rounded-lg border border-border bg-surface/70 px-3.5 py-2 text-[13px] font-semibold text-foreground transition-colors active:bg-muted disabled:opacity-60"
            >
              {checking ? (
        ...
        {groups.map((g) => (
          <section key={g.category} className="flex flex-col gap-1.5">
            ...
            {/* Single column on mobile — the desktop `sm:grid-cols-2` two-up
                cramps probe rows at phone width. */}
