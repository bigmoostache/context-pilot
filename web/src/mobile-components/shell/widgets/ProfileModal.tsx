...
// Shared text-input styling — mobile twin uses 16px text so iOS Safari doesn't
// zoom the viewport on focus (the desktop twin's 13px triggers focus-zoom on a
// phone). Padding bumped to py-2.5 for a comfortable tap target.
const INPUT_CLS =
  "w-full rounded-lg border border-border bg-card px-3 py-2.5 text-[16px] text-foreground outline-none transition-colors placeholder:text-muted-foreground/40 focus:border-signal focus:ring-1 focus:ring-signal"

/**
 * Profile modal — mobile twin of `components/shell/widgets/ProfileModal`.
 *
 * Same self-serve account management (edit name/email, change password, review
 * / revoke device sessions), all against the same RBAC endpoints. The fork is
 * presentation: the desktop centered `540px × 88vh` window becomes a
 * **full-screen sheet** (`w-screen h-screen`, no gutters) because a phone has no
 * room for a floating dialog, and every text field is 16px to defeat iOS
 * focus-zoom. Logic is identical.
 */
export function ProfileModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="flex h-screen max-h-screen w-screen max-w-none flex-col rounded-none border-0 p-0">
        <header className="flex items-start gap-3 border-b border-border/70 px-4 pt-4 pb-3">
          ...
          <DialogClose
            aria-label="Close"
            className="-mt-1 -mr-1 flex size-9 items-center justify-center rounded-md text-muted-foreground/55 transition-colors active:bg-muted/70 active:text-foreground"
          >
            ✕
          </DialogClose>
        </header>
        ...
        <footer className="flex items-center justify-end gap-2 border-t border-border/70 bg-muted/25 px-4 py-3">
          <DialogClose className="rounded-lg px-4 py-2.5 text-[13.5px] font-medium text-muted-foreground transition-colors active:bg-muted/70 active:text-foreground">
            Close
          </DialogClose>
        </footer>
      </DialogContent>
...
function IdentitySection({
  ...
  // Re-sync editable fields when the underlying user changes — render-phase
  // compare against last-seen values (React's canonical prop-change pattern),
  // not an effect (which would trip set-state-in-effect + cost an extra commit).
    ...
    <form onSubmit={(e) => void save(e)} className="flex flex-col gap-4">
      ...
      <div className="flex justify-end">
        <button
          type="submit"
          disabled={!dirty || busy}
          className="inline-flex items-center gap-1.5 rounded-lg bg-signal px-4 py-2.5 text-[14px] font-medium text-background transition-opacity active:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
        >
    ...
    >
      ...
      <div className="flex justify-end">
        <button
          type="submit"
          disabled={!canSubmit}
          className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-4 py-2.5 text-[14px] font-medium text-foreground transition-colors active:bg-muted/60 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {busy && <Loader2 className="size-3.5 animate-spin" />}
          Update password
        </button>
      ...
      {sessions === null ? (
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          {sessions.map((s) => (
            <div
              key={s.id}
              className="flex items-center gap-3 border-b border-border/60 bg-card px-3.5 py-3 last:border-0"
            >
              ...
              {!s.current && (
                <button
                  ...
                  className="flex size-9 shrink-0 items-center justify-center rounded-md text-muted-foreground/60 transition-colors active:bg-danger/10 active:text-danger"
                >
