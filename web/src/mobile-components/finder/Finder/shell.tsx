...
/**
 * Finder chrome + content render — mobile twin of `components/finder/Finder/
 * shell`. Same tab strip → toolbar → body → path bar → overlay composition and
 * the same disconnect gate; the touch divergence lives in the children (mobile
 * FinderChrome collapses the toolbar, mobile FinderBody drops the sidebar rail
 * for a drawer and runs a single-pane view). The `role="application"` +
 * `onKeyDown` shell is kept verbatim: it's harmless on touch and still serves a
 * paired hardware keyboard, while the actual navigation is tap-driven downstream.
 */
