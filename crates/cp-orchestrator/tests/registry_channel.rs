...
mod tests {
    ...
    /// Does `events` begin with an `Appeared` for `id`? Extracted from the tests
    /// so the assertion's `matches!` does not push them over the
    /// cognitive-complexity cap.
    fn first_is_appeared(events: &[Event], id: &str) -> bool {
        matches!(events.first(), Some(Event::Appeared(e)) if e.id == id)
    }
    ...
    fn a_real_agent_is_discovered_live_then_survives_graceful_shutdown() {
...

        {
            ...
            assert!(first_is_appeared(&first, &id), "the booted agent appears in the very first scan");
    ...
    fn stale_fires_once_on_transition_and_first_sight_staleness_rides_appeared() {
        ...
        let mut reg = FleetScanner::new(agents.path().to_path_buf());
        let first = reg.scan().expect("scan");
        assert!(first_is_appeared(&first, "a"));
    ...
    fn the_tailer_picks_up_a_spilled_body_the_channel_then_hydrates() {
        ...
        let head = entries
            .iter()
            .find_map(|e| if let &OpEntryKind::MessageCreated { head, .. } = &e.kind { Some(head) } else { None })
    ...
    fn a_command_round_trips_to_a_real_intake_and_stays_exactly_once() {
        ...
        let server = {
            let srv_oplog = oplog_dir.clone();
            let srv_token = token.clone();
            thread::spawn(move || {
                let oplog = OplogService::spawn(&srv_oplog).expect("spawn");
                let mut intake = Intake::new(&srv_oplog, srv_token).expect("intake");
                for _ in 0u32..2 {
