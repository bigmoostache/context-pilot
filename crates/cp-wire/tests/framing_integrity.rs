...
mod tests {
    ...
    /// Walk every whole frame a buffer contains, advancing by the reported
    /// consumed length. Returns the entries decoded from the clean prefix and the
    /// byte offset at which decoding stopped (the start of the first
    /// non-decodable frame, or the buffer length if every frame decoded).
