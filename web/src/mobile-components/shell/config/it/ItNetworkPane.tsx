 ...
 * mobile twin of `components/shell/config/it/ItNetworkPane`.
 *
 * Divergence from desktop is touch-only: the mode rows and the retry button
 * carry a taller hit area and `hover:` becomes `active:`. Everything else — the
 * query, the failure state, the mutation, the status/supervisor/probe rendering
 * — is byte-identical to the desktop twin, because every rule behind it lives in
 * the shared `@/lib/api/it/network*` modules rather than being forked (C8).
 *
      ...
      <PaneFrame>
        <div className="flex flex-col items-start gap-2 py-1">
          ...
          <button
            ...
            className="flex items-center gap-1 rounded-md px-3 py-1.5 text-[11px] text-muted-foreground transition-colors active:bg-muted/60 active:text-foreground"
          >
