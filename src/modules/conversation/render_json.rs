...
pub(super) fn extract_json_fields(partial: &str) -> Vec<(String, String)> {
    ...
    if let Ok(serde_json::Value::Object(map)) = serde_json::from_str(partial) {
        return fields_from_object(map);
    }
    hand_parse_fields(partial)
}

/// Convert a fully-parsed JSON object into flat `(key, value-string)` pairs.
fn fields_from_object(map: serde_json::Map<String, serde_json::Value>) -> Vec<(String, String)> {
    map.into_iter()
        .map(|(k, v)| {
            let val = match v {
...
/// Hand-parse key-value pairs from incomplete (still-streaming) JSON.
fn hand_parse_fields(partial: &str) -> Vec<(String, String)> {
...
/// Capture a nested `{...}` or `[...]` structure (from the current position to
/// its matching close, or end-of-input if incomplete) as a raw substring.
fn read_nested_structure(chars: &mut std::iter::Peekable<std::str::CharIndices<'_>>, full: &str, open: char) -> String {
...
/// Read a scalar (number, bool, null) up to the next delimiter.
fn read_json_scalar(chars: &mut std::iter::Peekable<std::str::CharIndices<'_>>) -> String {
...
fn read_json_value(chars: &mut std::iter::Peekable<std::str::CharIndices<'_>>, full: &str) -> String {
    match chars.peek().map(|&(_, c)| c) {
        ...
        Some(c) if c == '{' || c == '[' => read_nested_structure(chars, full, c),
        Some(_) => read_json_scalar(chars),
        None => String::new(),
    }
}
