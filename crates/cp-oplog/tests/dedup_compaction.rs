...
mod tests {
    ...
    fn build_rich_log(guard: &tempfile::TempDir) -> Vec<String> {
        ...
        // Thread rotation (every 3rd) and effect cadence (every 5th) are tracked with
        // explicit counters rather than `%` — the modulo operator is disallowed in
        // this foundational crate.
        ...
        for step in 0..21u8 {
            ...
            let _msg_rev = writer.append(msg(thread, step)).expect("append msg");

            if since_effect == 0 {
                let token = format!("tok-{step}");
                let _effect_rev = writer.append(effect(&token)).expect("append effect");
                tokens.push(token);
            }
            since_effect = if since_effect == 4 { 0 } else { since_effect.wrapping_add(1) };
        }
        tokens
    }
