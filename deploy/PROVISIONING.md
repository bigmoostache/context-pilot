...
- **`context-pilot` refuse de démarrer après 5 redémarrages dans l'heure.** L'unité porte `StartLimitBurst=5` / `StartLimitIntervalSec=3600` (garde-fou anti-crash-loop). En itération sur la box — déploiement de binaires de test, bascules de gates — on l'atteint vite, et Ansible échoue alors sur *« start of the service was attempted too often »*, ce qui **ressemble à un plantage sans en être un**. Remède : `systemctl reset-failed context-pilot` puis `systemctl start context-pilot`.
