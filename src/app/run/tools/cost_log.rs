...
const HEADER: &str = "datetime\tbefore_three_last_tools\tbefore_culprit_type\tbefore_tokens_before_culprit\tbefore_tokens_culprit\tbefore_tokens_after_culprit\tqueue_is_active\ttempo_is_active\tno_panel_broken\tafter_tokens_hit\tafter_cost_hit\tafter_tokens_miss\tafter_cost_miss\tafter_tokens_out\tafter_cost_out";
...
pub(crate) fn append_cost_tsv(state: &mut State) {
    ...
    let line = format!(
        "{datetime}\t{tools}\t{culprit}\t{before}\t{culp_tok}\t{after}\t{queue}\t{tempo}\t{no_break}\t{hit_tok}\t{hit_cost:.6}\t{miss_tok}\t{miss_cost:.6}\t{out_tok}\t{out_cost:.6}",
