...
mod tests {
    ...
    /// Spill an orphan body (never journalled) and assert it is durable and
    /// integrity-checked, yet unreferenced by any replayed head — the
    /// crash-in-the-gap precondition. Returns the orphan's spill handle.
    fn spill_unreferenced_orphan(store: &Store, dir: &std::path::Path) -> cp_mod_bridge::body::Stored {
        ...
        let recovered = replay(dir).expect("replay empty");
        assert!(recovered.heads.threads.is_empty(), "no entry references the orphan");
        orphan
    }

    /// Journal a `MessageCreated` referencing `head` into a fresh oplog life, so a
    /// later replay surfaces it in the live head set.
    fn journal_reference(dir: &std::path::Path, head: ContentHash) {
    ...
    fn the_i13_barrier_and_orphan_gc_hold_across_a_real_journal() {
        ...
        // never journalled.
        let orphan = spill_unreferenced_orphan(&store, dir.path());
        ...
        journal_reference(dir.path(), referenced.hash());

        // GC driven by the replayed heads — exactly how the agent reclaims bodies.
        let replayed = replay(dir.path()).expect("replay with reference");
        let live: HashSet<ContentHash> = replayed.heads.threads.iter().map(|h| h.last_message_hash).collect();
    ...
    /// Publish a `burst`-frame token stream onto `tee`, asserting nothing drops
    /// while within capacity.
    fn publish_burst(tee: &Tee, burst: u64) {
        for seq in 0..burst {
    ...
    /// Read `burst` framed [`StreamFrame`]s back off `observer`, decoding the
    /// shared length+CRC framing, and return their `seq`s in arrival order.
    fn drain_stream_seqs(observer: &mut UnixStream, burst: u64) -> Vec<u64> {
        let deadline = Instant::now().checked_add(Duration::from_secs(3)).expect("deadline in range");
        let mut buf: Vec<u8> = Vec::new();
        let mut chunk = [0u8; 512];
    ...
    fn the_tee_streams_an_ordered_multi_frame_burst() {
        ...
        let burst = 16u64;
        publish_burst(&tee, burst);

        let seqs = drain_stream_seqs(&mut observer, burst);
